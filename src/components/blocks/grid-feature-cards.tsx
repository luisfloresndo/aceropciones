...
export function FeatureCard({ feature, className, ...props }: FeatureCardPorps) {
	const p = genSeededPattern(feature.title);
...
/**
 * Patrón determinista (seeded) — evita desajustes de hidratación SSR.
 * Mismo `seed` produce siempre el mismo patrón en servidor y cliente.
 */
function genSeededPattern(seed: string, length = 5): number[][] {
	let s = 0;
